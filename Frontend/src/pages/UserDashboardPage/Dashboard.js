...
export default function DashboardHome() {
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-green-50 to-white text-[#0C2218]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        ...
        <StatCard title="My Profile" count="1" icon={<FaUser size={28} />} color="bg-blue-200" />
      </div>
...
function StatCard({ title, count, icon, color }) {
  return (
    <div
      className={`p-4 rounded-xl shadow-md ${color} flex flex-col items-center text-center md:flex-row md:items-start md:text-left gap-2 md:gap-4`}
    >
  ...
  return (
    <Link to={link} className="block">
      <div className="p-4 bg-white hover:bg-green-50 border border-green-100 rounded-xl shadow-md transition-transform duration-300 transform hover:-translate-y-1 hover:shadow-lg flex flex-col items-center text-center gap-2 md:items-start md:text-left">
        ...
        <h4 className="text-sm md:text-lg font-semibold text-[#0C2218]">
          {title}
        </h4>
      </div>
    </Link>
