...
export default function SolvedIssues() {
  ...
  const totalPages = Math.ceil(solved.length / itemsPerPage);
  const paginatedData = solved.slice(
    (page - 1) * itemsPerPage,
    page * itemsPerPage
  );

  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-green-50 to-white text-[#0C2218]">
      <div className="p-6 flex-grow">
        ...
        {solved.length > 0 ? (
          ...
          <p className="text-center text-gray-500 mt-10">
            You haven't solved any issues yet. Keep going! 🚀
          </p>
        )}
      </div>

      {/* Pagination ALWAYS pinned at bottom of the page */}
      <div className="py-4 flex justify-center gap-2 flex-wrap">
        <button
